...
|---------|---------|-------------|--------------|
| `texlive` | 2.0.5 | Installs TeX Live via the official TUG network installer | Shell (`install-tl`) |
| `gregorio` | 2.0.6 | Builds and installs Gregorio (Gregorian chant engraver) from source | **Autotools** (`autoreconf` + `./configure` + `make`) |
| `gregorio-lsp` | 1.0.5 | Builds and installs the Gregorio language server (`gregorio-lsp`, `grelint`, `grefmt`) from source | **Cargo** (`cargo build --release`) |
